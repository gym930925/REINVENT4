...
import torch
import torch_npu
# NPU 支持需要 torch_npu 扩展，优先尝试导入以注册 npu 后端
...
def _resolve_device(device):
    """校验并解析目标设备，支持 cpu/cuda/npu。

    若指定 npu 但 torch_npu 未安装或不可用，给出明确错误提示。
    """
    if device is None:
        return 'cpu'
    device = device.lower()
    if device.startswith('npu'):
        if not _HAS_NPU:
            raise RuntimeError(
                "指定了 NPU 设备，但未安装 torch_npu。请先安装 CANN 与 torch_npu。")
        if not torch.npu.is_available():
            raise RuntimeError(
                "NPU 不可用，请检查 Ascend 驱动/固件/CANN 是否正确安装。")
    return device


def main():
    """Main execution function."""
    # 解析并校验目标设备（cpu/cuda/npu）
    device = _resolve_device(args.device)
    if device.startswith('npu'):
        # 设置 NPU 默认设备，便于后续未显式指定 device 的算子
    ...
    # Load pre-trained weights（先加载到 CPU，再整体迁移到目标设备）
    stored = torch.load(args.store_dir, map_location='cpu', weights_only=False)
    body.load_state_dict(stored['model_state_dict'])
    # 显式将模型迁移到目标设备，NPU 必需此步骤
    body = body.to(device)
    body.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--device', type=str, default='cpu',
        help='Device to run model on (cpu/cuda/npu, e.g. npu:0)')
